...
grid <- expand.grid(interaction.depth=c(3,4,5), # Depth of variable interactions
                    n.trees=c(200,250,300,350,400,450,500,550,600,650,700,800),	        # Num trees to fit
                    shrinkage=c(0.05,0.01,0.02,0.05,0.1),		# Try 2 values for learning rate 
                    n.minobsinnode = c(500,1000,1500,2000,2500,3000))
#											
set.seed(1951)  # set the seed

gbm.tune <- train(x=trainX,y=trn2$BK,
                  method = "gbm",
                  metric = "ROC",
                  trControl = ctrl,
                  tuneGrid=grid,
